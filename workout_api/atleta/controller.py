...
from workout_api.atleta.schemas import AtletaAllOut, AtletaIn, AtletaOut, AtletaUpdate
...
async def query(
    ...
    cpf: Optional[str] = Query(None, description="CPF do atleta")
        ...
        if nome:
            filtros.append(AtletaModel.nome.icontains(f"%{nome}%"))
        if cpf:
            filtros.append(AtletaModel.cpf == cpf)
